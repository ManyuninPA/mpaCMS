...
class CIBlock{
    ...
    public static function Add($arrList){
        ...
        $DB->query('INSERT INTO `t_s_iblock` SET ?As', $arrList);
    ...
    public static function Update($id, $arrList){
        ...
        $time_now =  date ('Y-m-d H:i:s');
        $arrList['DATE_UPDATE'] = $time_now;
        $DB->query("UPDATE `t_s_iblock` SET ?As WHERE ID = '".intval($id)."'", $arrList);
        return $id;
    }
    public static function Delete($id){
        global $DB;
        $strSql = "DELETE FROM `t_s_iblock` WHERE ID = '".intval($id)."' limit 1";
        $DB->query($strSql);
        return true;
    }
